...
Class Action {
	...
	function save_price(){
		extract($_POST);
		foreach($cabin as $k =>$v){
			$v = str_replace(',', '', $v);
			$seat[$k] = str_replace(',', '', $seat[$k]);
			$berth[$k] = str_replace(',', '', $berth[$k]);
			$data= " station_from = $origin_station ";
			$data.= ", station_to = $k ";
			$data.= ", cabin_price = '$v' ";
			$data.= ", seat_price = '{$seat[$k]}' ";
			$data.= ", berth_price = '{$berth[$k]}' ";
	...
	function get_price(){
		...
		$get['cabin_price'] = isset($get['cabin_price']) ? $get['cabin_price'] : 0;
		$get['seat_price'] = isset($get['seat_price']) ? $get['seat_price'] : 0;
		$get['berth_price'] = isset($get['berth_price']) ? $get['berth_price'] : 0;
